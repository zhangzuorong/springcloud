...
public class OwnThread {
    ...
    //待参构造
    public OwnThread(int initPoolNum){
    ...
    public void execute(Runnable runnable){
        try{
            ...
            if(workThreadNum < threadNum){
                ...
                threads.add(myThead);
                workThreadNum++;
            }else {//线程池已满，放入任务队列，等待有空闲线程时执行
                if(!taskQueue.offer(runnable)){//将指定的元素插入到此队列的尾部（如果立即可行且不会超过该队列的容量），在成功时返回 true，如果此队列已满，则返回 false
                    rejectTask();
                }
            }
        }finally {
        ...
        public void run() {
            ...
            while (true) {
                ...
                //否则去任务队列取任务并执行
                    ...
                    Runnable queueTask = taskQueue.poll();//获取并移除此队列的头，如果此队列为空，则返回 null
                    if(queueTask != null)
                        queueTask.run();
                }
            }
    ...
    public static void main(String[] args) {
        OwnThread myThreadPool = new OwnThread(5);
